...
def parse_option():
    ...
    parser.add_argument('--method', type=str, default='Supervised', help='Baseline Supervised AT')
...
class Image_and_AugmentedTransform:
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        trainEvalDataset   = torchvision.datasets.CIFAR10(root='./data/' ,train=True,  transform=Image_and_AugmentedTransform(testTransform,trainEvalTransform),  download=True)
        testDataset        = torchvision.datasets.CIFAR10(root='./data/' ,train=False, transform=testTransform)
    elif opt.dataset == 'cifar100':
        trainEvalDataset   = torchvision.datasets.CIFAR100(root='./data/' ,train=True,  transform=Image_and_AugmentedTransform(testTransform,trainEvalTransform),  download=True)
...
def trainEvalNet_Ro(opt,trainEvalLoader,EvalNet,criterion,optimizer,device):
    ...
    for epoch in range(opt.numEpochs):
        for i, (X, labels) in enumerate(trainEvalLoader):
            ...
            delta = pgd_linf_end2end(EvalNet, x0, labels, opt.eps_AT, opt.alpha, opt.iter_AT)
            x0_adv = (x0 + delta)
            Z_x0 = EvalNet(x0_adv)
            loss1 = criterion(Z_x0, labels).to(device)
            Z_x1 = EvalNet(x1)
            loss2 = criterion(Z_x1, labels).to(device)
            loss = loss1 + loss2
